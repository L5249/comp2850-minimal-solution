...
- No loading indicator for HTMX requests (instant for now, but could be slow on poor network)
...
- Server-side validation catches blank titles (good), but no prevention of accidental delete
...
- Error message only appears after submission (could preview validation on blur)
